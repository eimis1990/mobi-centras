...
import { daysSince, renderSms, TEMPLATE_KEYS, type Order, type Settings, type TemplateKey } from '../lib/orders'
import { relDays, useT } from '../lib/i18n'
...
export function NotifyDialog({ order, settings, onSend, onClose }: Props) {
  ...
  const send = async () => {
    ...
    try { await onSend(text.trim()) } catch (e) { setError(e instanceof Error ? e.message : t('nd.failed')); setSending(false) }
  }
    ...
    <dialog ref={ref} onClose={onClose} onCancel={onClose} onKeyDown={e => e.key === 'Escape' && ref.current?.close()}
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-[15px] font-semibold">{isRepeat ? t('nd.reminder') : t('nd.title')}</h2>
        ...
        {isRepeat && order.notifiedAt && (
          <p className="mb-4 text-[12px] text-warning bg-warning/10 rounded-md px-3 py-2">{t('nd.lastSent', { when: relDays(t, daysSince(order.notifiedAt)) })}</p>
        )}
        ...
        <div className="flex items-center justify-between mt-1.5 text-[11px] text-muted-foreground tabular-nums">
          <span>{edited ? t('nd.edited') : t('nd.template', { name: t(`tpl.${key}`) })}</span>
          <span>{text.length} {t('common.chars')} · {segments(text)} SMS</span>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button type="button" disabled={!text.trim() || sending} onClick={send}
            ...
            <Send className="w-3.5 h-3.5" strokeWidth={2} /> {sending ? t('nd.sending') : t('nd.send')}
          </button>
        </div>
      </div>
    </dialog>
