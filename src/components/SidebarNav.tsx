...
export const navGroups = (ordersBadge?: number): NavGroupData[] => [
  {
    items: [
      { id: 'search', title: 'Search', icon: Search, shortcut: '⌘K' },
      { id: 'dashboard', title: 'Dashboard', icon: LayoutDashboard },
      { id: 'orders', title: 'Orders', icon: ShoppingBag, badge: ordersBadge || undefined },
...
function CompanyHeader({ name, subtitle }: { name: string; subtitle: string }) {
  return (
    <div className="flex items-center gap-3 px-2 py-2 mb-4 select-none">
      <div className="w-8 h-8 rounded-[6px] bg-primary text-primary-foreground flex items-center justify-center font-semibold text-[13px]">
        {name.charAt(0)}
      </div>
...
export function SidebarNav({ className = '', activeId, onSelect, company, subtitle, ordersBadge }: {
  className?: string; activeId: string; onSelect: (id: string) => void; company: string; subtitle: string; ordersBadge?: number
}) {
  return (
    <div className={`flex flex-col w-[260px] h-full bg-card/50 border-r border-border/50 p-3 font-sans ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-4 mt-2">
        {navGroups(ordersBadge).map((group, idx) => (
